...
В данном окне представлены значения параметров плеера по умолчанию. Эти значения сохраняются в конфигурационный файл настроек плеера _MultiPlayerNIIES.exe.config_, который располагается рядом с исполняемым файлом (_MultiPlayerNIIES.exe_) программы. На текущий момент в настройках можно настраивать:
